...
const PRODUCT_COLUMNS = "id,name,price,category,image_url,description,stock,created_at";
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const loadServerCart = useCallback(async (cartId: string) => {
    const { data, error } = await supabase
      .from("cart_items")
      .select("id, product_id, quantity, products(id,name,price,category,image_url,description,stock)")
      .eq("cart_id", cartId);
    if (error) throw error;
